...
def create_deb_repo(path):
    ...
    cmd = ['gpg', '--batch', '--no-tty', '--clearsign', '-o', 'InRelease', 'Release']
    ...
    cmd = ['gpg', '--batch', '--no-tty', '-abs', '-o', 'Release.gpg', 'Release']
    if not run_it(cmd, cwd=path, stdinput=''):
        return False
...
def create_rpm_repo(path):
    ...
    cmd = ['gpg', '--batch', '--no-tty', '--detach-sign', '--armor',
           'repodata/repomd.xml']
    if not run_it(cmd, cwd=path, stdinput=''):
        return False
...
def do_argparse(argv):
    ...
    parser.add_argument('-d', '--destdir', default=tempfile.gettempdir(),
