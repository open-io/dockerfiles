...
function prepare(){
  mkdir /run/gridinit
  install -d -o openio -g openio -m 750 /run/rawx/OPENIO/rawx-0
}
...
function update_rawx(){
  if [ ! -z "${RAWX_BETA}" ]; then
    echo "> Activate beta rawx"
    sed -i -e 's@/usr/sbin/httpd -D FOREGROUND  -f /etc/oio/sds/OPENIO/rawx-0/rawx-0-httpd.conf@/usr/bin/oio-rawx -s OIO,OPENIO,rawx,0 -f /etc/oio/sds/OPENIO/rawx-0/rawx-0-httpd.conf@' /etc/gridinit.d/OPENIO-rawx-0.conf
  fi
}
