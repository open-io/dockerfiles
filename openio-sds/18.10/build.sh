...
popd
# Logs to stdout
ansible node1 -i ansible-playbook-openio-deployment/products/sds/inventory.ini -m copy -a 'src=../commons/rsyslog.conf dest=/etc/rsyslog.d/openio-sds.conf mode=0644'
...
ansible node1 -i ansible-playbook-openio-deployment/products/sds/inventory.ini -m copy -a 'src=../commons/sds-healthcheck.sh dest=/usr/local/bin/sds-healthcheck.sh mode=0755'
